...
public class EditPrimativeArray extends JFrame {
	...
	public EditPrimativeArray(ObjectToAdd obj,DefaultListModel list, String name) {
		super(name);
		preExistingObj = list;
	...
	private static JPanel giveArraylength() 
	{
		...
		displayPanel.add(theField);
		displayPanel.add(theButton);
		...
		addFieldButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				if (myObject.getObjectArrayLength() > 0 && (counter < MAX_OBJ)) {
					...
					myObject.addPrimitiveToArray(newObjectToAdd);
					String valueString = "Double Click here to Edit";
					fieldValueList.addElement(valueString);
					counter++;
				}
